...
def lines_intersect(p1, p2, p3, p4):
    ...
    try:
        A, B = (float(p1[1]), float(p1[0])), (float(p2[1]), float(p2[0]))  # (lon, lat) for calculation
        C, D = (float(p3[1]), float(p3[0])), (float(p4[1]), float(p4[0]))
    except (TypeError, ValueError, IndexError):
        return False
...
def line_intersects_polygon(p1, p2, polygon):
    ...
    for i in range(n):
        try:
            p3 = (float(polygon[i][0]), float(polygon[i][1]))
            p4 = (float(polygon[(i+1) % n][0]), float(polygon[(i+1) % n][1]))
        except (TypeError, ValueError, IndexError):
            continue
